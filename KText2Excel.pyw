...
def DoubleExe(): # проверка на уже запущеное приложение
    ...
    if list == []: # если нет найденых названий программы
        program_directory = os.path.dirname(os.path.abspath(__file__)) # ничего не указывааем (будет рядом с программой)
...
def ExcelAPI(): # подключение API Excel (https://memotut.com/en/150745ae0cc17cb5c866/)
    ...
    def SettingsExcel(wb): # настройки считанные с Excel
        ...
        try: # попытаться открыть файл Excel
            ...
            max_row = ws.Cells(ws.Rows.Count, 1).End(xlUp).Row # максимальное кол-во строк по 1-й колонке

            for parameter in ws.Range(f"A1:C{max_row}").Value: # считывание значений из Excel

                if parameter[1] != None: # есои параметр не пустой
    ...
    def ClearRows(ws): # очистка строк если уже заполнен Excel
        ...
        max_row = ws.Cells(ws.Rows.Count, 1).End(xlUp).Row # максимальное кол-во строк по 1-й колонке
        max_col = ws.Cells(1, ws.Columns.Count).End(xlToLeft).Column # максимальное кол-во колонок 1-й строчки
    ...
    def CreateSettingsExcel(wb): # сохдать лист с настройками в Excel
        ...
        ws = wb.Worksheets(2) # выбираем первый лист (создается автоматом)
        ws.Name = "Настройки" # переимненовать лист

        n = 1 # отсчёт строки

        for parameter, val in dict_settings.items(): # считывание значений из Excel

            ws.Range(f"A{n}:C{n}").Value = (parameter, val[0], val[1]) # прописываем значения

            ws.Range(f"A{n}:C{n}").EntireColumn.AutoFit() # автоширина ячейки
    ...
    if os.path.exists(name_txt_file): # если есть txt файл использовать его
        ...
        try: # попытаться открыть файл Excel
            ...
            SettingsExcel(wb) # настройки считанные с Excel
...
def Settings(): # присвоене значений параметров
    ...
    check_update = dict_settings.setdefault("check_update")[0] # опция проверки обновления программы
    beta = dict_settings.setdefault("beta")[0] # опция скачивания бета версии программы
...
def Record2Excel(text): # запись в Excel
    ...
    try: # попытаться записать значение в Excel
        ...
        if start_row > 6: # ограничение в 6 строк
            stop = True # тригер остановки
